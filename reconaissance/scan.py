...
def scan(place_name, verbose=False):
    if place_name == "outremont":
        ...
        place_name = "Saint-Léonard, Montreal, Canada"
        pass
    else:
        print(place_name, " n'est pas un emplacement gerer dans ce projet")
        print(
            "Les quartiers gerer sont : [outremont,verdun,montRoyal,riviere,saintLeonard]")
        return
    ...
    if verbose:
        ...
        print("Creation du plan avec l'itineraire (Cette operation peut etre longue) ETA :",
              0.08 * len(chemin_parcouru), " secondes")
        # Tracer l'itinéraire sur OSMnx
        start_time = time.time()
        ox.plot_graph_routes(
            GU, chemin_parcouru, route_linewidth=6, node_size=0, bgcolor="w", show=True)
        end_time = time.time()
        execution_time = end_time - start_time  # Calcul du temps d'exécution
        print("Temps d'exécution : ", execution_time,
              " secondes avec ", len(chemin_parcouru), " arretes")
    print("Fin du scan")
...
def dfs(graph, node, visited):
    ...
    while stack:
        ...
        if len(visited) > 1 and graph.degree(current_node) % 2 != 0:
            # Retourner le nœud, la distance et le chemin
            return (current_node, distance, path)
        for neighbor in graph.neighbors(current_node):
            if neighbor not in visited:
                edge_length = graph.get_edge_data(
                    current_node, neighbor)[0]["length"]
                stack.append(
                    (neighbor, distance + edge_length, path + [neighbor]))
...
def make_it_eulerian(graph):
    ...
    while odd_nodes:
        ...
        odd_degree_neighbors = [
            neighbor
            for neighbor in graph.neighbors(node)
            if graph.degree(neighbor) % 2 != 0
        ]
        if odd_degree_neighbors and odd_degree_neighbors[0] != node:
            length = graph.get_edge_data(
                odd_degree_neighbors[0], node)[0]["length"]
            graph.add_edge(
                node, odd_degree_neighbors[0], directed=False, length=length)
            added_edge += 1
        else:
            ...
            for i in range(len(path) - 1):
                ...
                edge_length = graph.get_edge_data(
                    source, destination)[0]["length"]
                graph.add_edge(source, destination,
                               directed=False, length=edge_length)
            added_edge += 1
        odd_nodes = [
            node for node in graph.nodes if graph.degree(node) % 2 != 0]
    if not odd_nodes:
...
def parcourir_aretes_euler(graph):
    ...
    distance_totale = sum(graph.get_edge_data(
        u, v)[0]["length"] for u, v in cycle_arretes)
